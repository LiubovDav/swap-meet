...
- `Vendor`s have a method named `swap_best_by_category`, which will swap the best item of certain categories with another `Vendor`
  - It takes in three arguments
    ...
    - `my_category`, which represents a category that the `Vendor` wants to receive
    - `their_category`, which represents a category that `other_vendor` wants to receive
  - The best item in my inventory that matches `their_category` category is swapped with the best item in `other_vendor`'s inventory that matches `my_category`
    - It returns `True`
    - If the `Vendor` has no item that matches `their_category` category, swapping does not happen, and it returns `False`
    - If `other_vendor` has no item that matches `my_category` category, swapping does not happen, and it returns `False`
